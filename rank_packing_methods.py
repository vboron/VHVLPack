...
def rank_methods():
    ...
    df['combined-para'] = (1/df['pearson']) + df['error'].abs() + df['RMSE'].abs() + \
        df['RELRMSE'].abs() + df['slope'].abs() + df['intercept'].abs()
    df = df.sort_values(by=['combined-para'], key=abs)
    top10 = df.head(10)

    df_o = pd.concat(data_o)
    df_o['combined-para'] = (1/df_o['pearson']) + df_o['error'].abs() + df_o['RMSE'].abs() + \
        df_o['RELRMSE'].abs() + df_o['slope'].abs() + df_o['intercept'].abs()
    df_o = df_o.sort_values(by=['combined-para'], key=abs)
