...
def find_normal_and_outliers():
    ...
    df_n.to_csv('normal{}'.format(sys.argv[3]), index=False)
    df_o = df_o.reset_index()
    df_o.to_csv('outliers{}'.format(sys.argv[3]), index=False)
...
def find_stats(df_o):
    ...
    getResult = lambda rmse: subprocess.check_output(['./RELRMSE.py', 'all_{}.csv'.format(sys.argv[3]), 'graph.dat', rmse])
...
def plot_scatter(file_o, file_n, stat_df, file_a):
    ...
    # Adds graph title
    plt.suptitle('ML prediction of VHVL packing angle', fontsize=14)
    ...
    stat_df.to_csv('stats_for_run({}).csv'.format(sys.argv[3]), index=False)

    # Exports the figure as a .png file
    plt.savefig('{}.tiff'.format(sys.argv[3]), format='tiff')
    plt.show()
